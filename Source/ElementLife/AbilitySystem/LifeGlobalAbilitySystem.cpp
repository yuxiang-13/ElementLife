...
void FGlobalAppliedAbilityList::AddToASC(TSubclassOf<UGameplayAbility> Ability, ULifeAbilitySystemComponent* ASC)
{
...
void FGlobalAppliedAbilityList::RemoveFromASC(ULifeAbilitySystemComponent* ASC)
{
	if (FGameplayAbilitySpecHandle* SpecHandle = Handles.Find(ASC))
	{
...
void FGlobalAppliedEffectList::AddToASC(TSubclassOf<UGameplayEffect> Effect, ULifeAbilitySystemComponent* ASC)
{
...
void FGlobalAppliedEffectList::RemoveFromASC(ULifeAbilitySystemComponent* ASC)
{
	if (FActiveGameplayEffectHandle* EffectHandle = Handles.Find(ASC))
	{
...
ULifeGlobalAbilitySystem::ULifeGlobalAbilitySystem()
{
}

void ULifeGlobalAbilitySystem::ApplyAbilityToAll(TSubclassOf<UGameplayAbility> Ability)
{
	if ((Ability.Get() != nullptr) && (!AppliedAbilities.Contains(Ability)))
	{
		FGlobalAppliedAbilityList& Entry = AppliedAbilities.Add(Ability);		
		for (ULifeAbilitySystemComponent* ASC : RegisteredASCs)
		{
...
void ULifeGlobalAbilitySystem::ApplyEffectToAll(TSubclassOf<UGameplayEffect> Effect)
{
...
void ULifeGlobalAbilitySystem::RemoveAbilityFromAll(TSubclassOf<UGameplayAbility> Ability)
{
	if ((Ability.Get() != nullptr) && AppliedAbilities.Contains(Ability))
	{
...
void ULifeGlobalAbilitySystem::RemoveEffectFromAll(TSubclassOf<UGameplayEffect> Effect)
{
	if ((Effect.Get() != nullptr) && AppliedEffects.Contains(Effect))
	{
...
void ULifeGlobalAbilitySystem::RegisterASC(ULifeAbilitySystemComponent* ASC)
{
	check(ASC);
...
void ULifeGlobalAbilitySystem::UnregisterASC(ULifeAbilitySystemComponent* ASC)
{
	check(ASC);
	for (auto& Entry : AppliedAbilities)
	{
