...
#include "LifeGameplayCueManager.h"
...
enum class ELifeEditorLoadMode
{
...
namespace LifeGameplayCueManagerCvars
{
	static FAutoConsoleCommand CVarDumpGameplayCues(
		TEXT("Life.DumpGameplayCues"),
		TEXT("Shows all assets that were loaded via LifeGameplayCueManager and are currently in memory."),
		FConsoleCommandWithArgsDelegate::CreateStatic(ULifeGameplayCueManager::DumpGameplayCues));

	static ELifeEditorLoadMode LoadMode = ELifeEditorLoadMode::LoadUpfront;
}
...
ULifeGameplayCueManager::ULifeGameplayCueManager(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

ULifeGameplayCueManager* ULifeGameplayCueManager::Get()
{
	return Cast<ULifeGameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
}

void ULifeGameplayCueManager::OnCreated()
{
	Super::OnCreated();
...
void ULifeGameplayCueManager::LoadAlwaysLoadedCues()
{
	if (ShouldDelayLoadGameplayCues())
	{
		...
		for (const FName& CueTagName : AdditionalAlwaysLoadedCueTags)
		{
			...
			{
				UE_LOG(LogLife, Warning, TEXT("ULifeGameplayCueManager::AdditionalAlwaysLoadedCueTags contains invalid tag %s"), *CueTagName.ToString());
			}
		}
	}
}

bool ULifeGameplayCueManager::ShouldAsyncLoadRuntimeObjectLibraries() const
{
	switch (LifeGameplayCueManagerCvars::LoadMode)
	{
	case ELifeEditorLoadMode::LoadUpfront:
		return true;
	case ELifeEditorLoadMode::PreloadAsCuesAreReferenced_GameOnly:
#if WITH_EDITOR
		if (GIsEditor)
		{
			return false;
...
#endif
		break;
	case ELifeEditorLoadMode::PreloadAsCuesAreReferenced:
		break;
	}
...
bool ULifeGameplayCueManager::ShouldSyncLoadMissingGameplayCues() const
{
	return false;
}

bool ULifeGameplayCueManager::ShouldAsyncLoadMissingGameplayCues() const
{
	return true;
}

void ULifeGameplayCueManager::DumpGameplayCues(const TArray<FString>& Args)
{
	ULifeGameplayCueManager* GCM = Cast<ULifeGameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
	if (!GCM)
	{
		UE_LOG(LogLife, Error, TEXT("DumpGameplayCues failed. No ULifeGameplayCueManager found."));
		return;
	}

	const bool bIncludeRefs = Args.Contains(TEXT("Refs"));

	UE_LOG(LogLife, Log, TEXT("=========== Dumping Always Loaded Gameplay Cue Notifies ==========="));
	for (UClass* CueClass : GCM->AlwaysLoadedCues)
	{
		UE_LOG(LogLife, Log, TEXT("  %s"), *GetPathNameSafe(CueClass));
	}

	UE_LOG(LogLife, Log, TEXT("=========== Dumping Preloaded Gameplay Cue Notifies ==========="));
	for (UClass* CueClass : GCM->PreloadedCues)
	{
		...
		UE_LOG(LogLife, Log, TEXT("  %s (%d refs)"), *GetPathNameSafe(CueClass), NumRefs);
		if (bIncludeRefs && ReferencerSet)
		{
			for (const FObjectKey& Ref : *ReferencerSet)
			{
				UObject* RefObject = Ref.ResolveObjectPtr();
				UE_LOG(LogLife, Log, TEXT("    ^- %s"), *GetPathNameSafe(RefObject));
			}
		}
	}

	UE_LOG(LogLife, Log, TEXT("=========== Dumping Gameplay Cue Notifies loaded on demand ==========="));
	...
	if (GCM->RuntimeGameplayCueObjectLibrary.CueSet)
	{
		for (const FGameplayCueNotifyData& CueData : GCM->RuntimeGameplayCueObjectLibrary.CueSet->GameplayCueData)
		{
			if (CueData.LoadedGameplayCueClass && !GCM->AlwaysLoadedCues.Contains(CueData.LoadedGameplayCueClass) && !GCM->PreloadedCues.Contains(CueData.LoadedGameplayCueClass))
			{
				...
				UE_LOG(LogLife, Log, TEXT("  %s"), *CueData.LoadedGameplayCueClass->GetPathName());
			}
		}
	}

	UE_LOG(LogLife, Log, TEXT("=========== Gameplay Cue Notify summary ==========="));
	UE_LOG(LogLife, Log, TEXT("  ... %d cues in always loaded list"), GCM->AlwaysLoadedCues.Num());
	UE_LOG(LogLife, Log, TEXT("  ... %d cues in preloaded list"), GCM->PreloadedCues.Num());
	UE_LOG(LogLife, Log, TEXT("  ... %d cues loaded on demand"), NumMissingCuesLoaded);
	UE_LOG(LogLife, Log, TEXT("  ... %d cues in total"), GCM->AlwaysLoadedCues.Num() + GCM->PreloadedCues.Num() + NumMissingCuesLoaded);
}

void ULifeGameplayCueManager::OnGameplayTagLoaded(const FGameplayTag& Tag)
{
		...
		TGraphTask<FGameplayCueTagThreadSynchronizeGraphTask>::CreateTask().ConstructAndDispatchWhenReady([]()
			{
				if (GIsRunning)
				{
					if (ULifeGameplayCueManager* StrongThis = Get())
					{
...
void ULifeGameplayCueManager::HandlePostGarbageCollect()
{
...
void ULifeGameplayCueManager::ProcessLoadedTags()
{
	...
	if (GIsRunning)
	{
		...
		{
			UE_LOG(LogLife, Warning, TEXT("ULifeGameplayCueManager::OnGameplayTagLoaded processed loaded tag(s) but RuntimeGameplayCueObjectLibrary.CueSet was null. Skipping processing."));
		}
	}
}

void ULifeGameplayCueManager::ProcessTagToPreload(const FGameplayTag& Tag, UObject* OwningObject)
{
	switch (LifeGameplayCueManagerCvars::LoadMode)
	{
	case ELifeEditorLoadMode::LoadUpfront:
...
void ULifeGameplayCueManager::OnPreloadCueComplete(FSoftObjectPath Path, TWeakObjectPtr<UObject> OwningObject, bool bAlwaysLoadedCue)
{
...
void ULifeGameplayCueManager::RegisterPreloadedCue(UClass* LoadedGameplayCueClass, UObject* OwningObject)
{
...
void ULifeGameplayCueManager::HandlePostLoadMap(UWorld* NewWorld)
{
	if (RuntimeGameplayCueObjectLibrary.CueSet)
	{
...
void ULifeGameplayCueManager::UpdateDelayLoadDelegateListeners()
{
...
bool ULifeGameplayCueManager::ShouldDelayLoadGameplayCues() const
{
...
void ULifeGameplayCueManager::RefreshGameplayCuePrimaryAsset()
{
