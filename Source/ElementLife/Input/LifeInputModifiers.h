...
#include "LifeInputModifiers.generated.h"

class UEnhancedPlayerInput;
class ULifeAimSensitivityData;
...
class ULifeSettingBasedScalar : public UInputModifier
{
	GENERATED_BODY()
 ...
 */
UCLASS(NotBlueprintable, MinimalAPI, meta = (DisplayName = "Life Settings Driven Dead Zone"))
class ULifeInputModifierDeadZone : public UInputModifier
{
	GENERATED_BODY()
...
UENUM()
enum class ELifeTargetingType : uint8
{
...
/** Applies a scalar modifier based on the current gamepad settings in Life Shared game settings.  */
UCLASS(NotBlueprintable, MinimalAPI, meta = (DisplayName = "Life Gamepad Sensitivity"))
class ULifeInputModifierGamepadSensitivity : public UInputModifier
{
	GENERATED_BODY()
public:
	...
	UPROPERTY(EditInstanceOnly, BlueprintReadWrite, Category=Settings, Config)
	ELifeTargetingType TargetingType = ELifeTargetingType::Normal;

	/** Asset that gives us access to the float scalar value being used for sensitivty */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, meta=(AssetBundles="Client,Server"))
	TObjectPtr<const ULifeAimSensitivityData> SensitivityLevelTable;

protected:
	virtual FInputActionValue ModifyRaw_Implementation(const UEnhancedPlayerInput* PlayerInput, FInputActionValue CurrentValue, float DeltaTime) override;
};

/** Applies an inversion of axis values based on a setting in the Life Shared game settings */
UCLASS(NotBlueprintable, MinimalAPI, meta = (DisplayName = "Life Aim Inversion Setting"))
class ULifeInputModifierAimInversion : public UInputModifier
{
	GENERATED_BODY()
