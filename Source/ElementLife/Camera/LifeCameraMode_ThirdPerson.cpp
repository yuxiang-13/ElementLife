...
#include "LifeCameraMode_ThirdPerson.h"
#include "Curves/CurveVector.h"
#include "Engine/Canvas.h"
#include "GameFramework/CameraBlockingVolume.h"
#include "LifeCameraAssistInterface.h"
#include "GameFramework/Controller.h"
#include "GameFramework/Character.h"

namespace LifeCameraMode_ThirdPerson_Statics
{
	static const FName NAME_IgnoreCameraCollision = TEXT("IgnoreCameraCollision");
}

ULifeCameraMode_ThirdPerson::ULifeCameraMode_ThirdPerson()
{
	TargetOffsetCurve = nullptr;

	PenetrationAvoidanceFeelers.Add(FLifePenetrationAvoidanceFeeler(FRotator(+00.0f, +00.0f, 0.0f), 1.00f, 1.00f, 14.f, 0));
	PenetrationAvoidanceFeelers.Add(FLifePenetrationAvoidanceFeeler(FRotator(+00.0f, +16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FLifePenetrationAvoidanceFeeler(FRotator(+00.0f, -16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FLifePenetrationAvoidanceFeeler(FRotator(+00.0f, +32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FLifePenetrationAvoidanceFeeler(FRotator(+00.0f, -32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FLifePenetrationAvoidanceFeeler(FRotator(+20.0f, +00.0f, 0.0f), 1.00f, 1.00f, 00.f, 4));
	PenetrationAvoidanceFeelers.Add(FLifePenetrationAvoidanceFeeler(FRotator(-20.0f, +00.0f, 0.0f), 0.50f, 0.50f, 00.f, 4));
}

void ULifeCameraMode_ThirdPerson::UpdateView(float DeltaTime)
{
...
void ULifeCameraMode_ThirdPerson::UpdateForTarget(float DeltaTime)
{
...
void ULifeCameraMode_ThirdPerson::DrawDebug(UCanvas* Canvas) const
{
...
void ULifeCameraMode_ThirdPerson::UpdatePreventPenetration(float DeltaTime)
{
	...
	ILifeCameraAssistInterface* TargetControllerAssist = Cast<ILifeCameraAssistInterface>(TargetController);

	ILifeCameraAssistInterface* TargetActorAssist = Cast<ILifeCameraAssistInterface>(TargetActor);
	...
	ILifeCameraAssistInterface* PPActorAssist = OptionalPPTarget.IsSet() ? Cast<ILifeCameraAssistInterface>(PPActor) : nullptr;
	...
	if (PPActorRootComponent)
	{
		...
		ILifeCameraAssistInterface* AssistArray[] = { TargetControllerAssist, TargetActorAssist, PPActorAssist };

		if (AimLineToDesiredPosBlockedPct < ReportPenetrationPercent)
		{
			for (ILifeCameraAssistInterface* Assist : AssistArray)
			{
				if (Assist)
				{
...
void ULifeCameraMode_ThirdPerson::PreventCameraPenetration(class AActor const& ViewTarget, FVector const& SafeLoc, FVector& CameraLoc, float const& DeltaTime, float& DistBlockedPct, bool bSingleRayOnly)
{
#if ENABLE_DRAW_DEBUG
	...
	//TODO ILifeCameraTarget.GetIgnoredActorsForCameraPentration();
	...
	{
		FLifePenetrationAvoidanceFeeler& Feeler = PenetrationAvoidanceFeelers[RayIdx];
		if (Feeler.FramesUntilNextTrace <= 0)
		{
			...
			if (bHit && HitActor)
			{
				...
				if (HitActor->ActorHasTag(LifeCameraMode_ThirdPerson_Statics::NAME_IgnoreCameraCollision))
				{
					bIgnoreHit = true;
...
void ULifeCameraMode_ThirdPerson::SetTargetCrouchOffset(FVector NewTargetOffset)
{
...
void ULifeCameraMode_ThirdPerson::UpdateCrouchOffset(float DeltaTime)
{
