...
#define LOCTEXT_NAMESPACE "LifeGameFeatures"
...
{
	for (const FLifeHUDElementEntry& Entry : Widgets)
	{
...
EDataValidationResult UGameFeatureAction_AddWidgets::IsDataValid(TArray<FText>& ValidationErrors)
{
	...
	{
		int32 EntryIndex = 0;
		for (const FLifeHUDLayoutRequest& Entry : Layout)
		{
...
void UGameFeatureAction_AddWidgets::AddToWorld(const FWorldContext& WorldContext, const FGameFeatureStateChangeContext& ChangeContext)
{
	...
	if ((GameInstance != nullptr) && (World != nullptr) && World->IsGameWorld())
	{
		...
		{			
			TSoftClassPtr<AActor> HUDActorClass = ALifeHUD::StaticClass();
...
{
	ALifeHUD* HUD = CastChecked<ALifeHUD>(Actor);
